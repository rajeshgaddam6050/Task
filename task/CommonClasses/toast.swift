...
//  ChatApp
//
//  Created by KoniReddy on 27/12/17.
...
extension UIView
{
    func showToast(toastMessage:String,duration:CGFloat)
    {
        ...
        lblMessage.backgroundColor = UIColor.black.withAlphaComponent(0.6)
        lblMessage.textAlignment = .center
        lblMessage.font = UIFont(name: "Montserrat-Light", size: 13.0)
       ...
       // lblMessage.frame = CGRect(x:((self.bounds.size.width)/2) - ((expectedSizeTitle.width+16)/2) , y: (self.bounds.size.height/2) - ((expectedSizeTitle.height+16)/2), width: expectedSizeTitle.width+16, height: expectedSizeTitle.height+16)
        ...
        lblMessage.frame = CGRect(x:((self.bounds.size.width)/2) - ((expectedSizeTitle.width+16)/2) , y: bgView.frame.size.height-150, width: expectedSizeTitle.width+16, height: expectedSizeTitle.height+16)
